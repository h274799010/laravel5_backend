                    ...
                    <li>
                        <ul class="dashboard-menu-<?php echo $key;?> nav nav-list collapse <?php if($checkFirst or $lessThenThree) echo 'in'; ?>">
                            <?php foreach($value[$son] as $skey => $svalue): ?>
                                ...
                                <?php if(isset($svalue[$son]) && is_array($svalue[$son]) && !empty($svalue[$son])): ?>
                                <?php foreach($svalue[$son] as $sskey => $ssvalue): ?>
                                <?php $checkThird = $MCA->matchThirdMenu($ssvalue['module'], $ssvalue['class'], $ssvalue['action']); ?>
                                    <?php $surl = R('common', $ssvalue['module'] .'.'. $ssvalue['class'] .'.'. $ssvalue['action']); ?>
                                <li <?php if($checkThird): ?>style="border-left:16px solid #8989a8;overflow:hidden;"<?php endif; ?>>
                                    <a href="<?php echo $surl; ?>" class="nav-sub-menu" <?php if($checkThird): ?>style="background:#d2d2df;margin-left:-16px"<?php endif; ?>>
                                         <?php echo '┆┄'.$ssvalue['name']; ?>
                                    </a>
                                </li>
