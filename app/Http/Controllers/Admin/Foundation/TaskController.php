...
use App\Services\Admin\Task\Process as TaskActionProcess;
...
class TaskController extends Controller
{
    ...
    public function index()
    {
        ...
        if($status!=''){
            //dd(11);
            $list = (new TaskModel())->myTask(Request::input('status'));
            $page = $list->setPath('')->appends(Request::all())->render();

        }else {
            //dd(22);
            $list = (new TaskModel())->allTask();
            $page = $list->setPath('')->appends(Request::all())->render();
        }
        //dd($list);
    ...
    public function read()
    {
        //if(Request::method() == 'POST') return $this->saveDatasToDatabase();
        $id=Request::Input('id');
        $formUrl = R('common', 'foundation.task.read');
       // dd($id);
        $info=(new TaskModel())->getOneById($id);
        $user_id=SC::getLoginSession()->id;


        return view('admin.task.read', compact('formUrl','info','user_id'));
    }
    public function letmedo()
    {
        $id=Request::Input('id');

        $data['exe_id']=SC::getLoginSession()->id;
        $data['exe_name']=SC::getLoginSession()->name;
        if((new TaskModel())->editTask($data,$id))
        {
            $formUrl = R('common', 'foundation.task.read');
            // dd($id);
            $info=(new TaskModel())->getOneById($id);
